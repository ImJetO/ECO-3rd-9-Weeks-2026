...
def mining((inventory, gem_inventory)):
    ...
    mining(inventory, gem_inventory, gemstones, rarity)
    print("You head down to the mines...")

    if inventoryPick == "basic":
        start_percent = 30

    if inventoryPick == "good":
        start_percent = 50

    if inventoryPick == "better":
        start_percent = 70
    ...
    while keepSwinging == "y" and keepSwinging == "Y":
        swing = input("Swing? (Y/N)")

        if swing == "y" or swing == "Y":
            print("You reveal a little more gemstones...")
            chance = chance + 5
            ...
            if roll <= chance:
                gem_name = gemstones[random.random(length, (gemstones))]
                gem_rarity = [random.random(1, length, (rarity))]
                print(f"You uncover a {gem_name}!")
